# gantry — Project Context for Codex

## What this project is

**gantry** is an OSS Codex skill package that keeps engineers in the design loop while AI writes code, so the code stays comprehensible to its author (and onboarders) months later. One primitive (collaborative pseudocode + AI surfaces edge cases + engineer approves + AI writes body) running in two directions:
...
OSS, MIT, no revenue plan in v0. Distribution: Codex skill ecosystem (gstack/gsd-shaped).
...
The convention: this AGENTS.md is the single discoverable entry point for any future Codex (or human) session. It must always point at the freshest artifacts.
...
- Language: TypeScript (skills) — to match Codex skill conventions
- Runtime: Bun preferred, Node fallback
- License: MIT
- Distribution: Codex skill ecosystem
